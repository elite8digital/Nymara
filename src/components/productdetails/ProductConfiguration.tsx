...
const ProductConfiguration: React.FC<ProductConfigurationProps> = ({
  ...
  // ✅ SAFE NORMALIZATION
  const metalBases = product.metalBases ?? [];
      ...
      standardSizes.length > 1 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => setShowSizeGuide(true)}
              className="text-sm text-[#9a8457] flex items-center gap-1"
            >
              <Ruler className="w-4 h-4" />
              Size Guide
            </button>
  ...
  return (
    <div className="space-y-6">
      ...
      {metalBases.length > 0 && (
        <div>
          ...
          <div className="flex gap-3">
            {metalBases.map((m) => {
              const isActive = m.purity === product.currentPurity;
              ...
              return (
                <button
                  ...
                  className={`px-6 py-2 rounded-lg border font-semibold tracking-wide transition-all
                    ${
                      isActive
                        ? "bg-[#9a8457] text-white border-[#9a8457] shadow-md"
                        : "bg-white text-gray-800 border-gray-300 hover:border-[#9a8457] hover:text-[#9a8457]"
                    }`}
                >
                  {m.purity}
                </button>
      ...
      <div>
        <button
          onClick={() => setShowEngravingForm(!showEngravingForm)}
          className="flex items-center gap-2 text-[#9a8457]"
        >
          <Plus className="w-4 h-4" />
        ...
        {showEngravingForm && (
          <div className="mt-3 p-4 bg-gray-50 rounded-lg">
            <input
              ...
              className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-[#9a8457]"
            />
            <div className="text-xs text-gray-500 mt-1">
              {engravingText.length}/10 characters
            </div>
